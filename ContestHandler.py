...
class ContestHandlerCommand(sublime_plugin.TextCommand):
    def create_path(self, base, _path):
        base = "E:\WorkPlace_Algorithm\code\CodeForce"
        for i in range(len(_path)):
            cur = path.join(base, path.join(*_path[:i + 1]))
            if not path.exists(cur):
                os.mkdir(cur)
        # path = "E:\WorkPlace_Algorithm\code\CodeForce"
        return path.join(base, path.join(*_path))
        # return "E:\WorkPlace_Algorithm\code\CodeForce"

    def next_problem(self, pid):
        if len(pid) == 1:
            return chr(ord(pid[0]) + 1)
    ...
    def init_problems(self, handler, contest_id, base, pid='A'):
        ...
        if inputs:
            ...
            if len(pid) == 1:
                ...
                self.init_problems(handler, contest_id,
                                   base, chr(ord(pid[0]) + 1))
    ...
    def init_contest(self, handler, contest_id):
        ...
        base = self.create_path(path.expanduser(
            '~'), [handler.get_contest_info(contest_id)['title']])
        sublime.run_command('close_window')
