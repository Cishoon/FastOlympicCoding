...
class TestManagerCommand(sublime_plugin.TextCommand):
    ...
    REGION_END_PROP = ['variable.c++', 'Packages/' +
                       base_name + '/icons/arrow_left.png', sublime.HIDDEN]
    ...
    def __init__(self, view):
        ...
        self.test_phantoms = [PhantomSet(
            view, 'test-phantoms-' + str(i)) for i in range(10)]
    ...
    class Test(object):
        ...
        def __init__(self, prop, start=None, end=None):
            ...
            if type(prop) == str:
                ...
                self.uncorrect_answers = set(
                    prop.get('uncorrect_answers', set()))
        ...
        def get_config(self, i, pt, _cb_act, _out, view, running=False):
            if not running:
                ...
                phantom = Phantom(Region(pt), content,
                                  sublime.LAYOUT_BLOCK, onclick)
                return phantom
            else:
                styles = get_test_styles(view)
                content = open(
                    root_dir + '/Highlight/test_running.html').read()
                content = content.format(
                    test_id=i
                )
                content = '<style>' + styles + '</style>' + content
        ...
        def get_accdec(self, i, pt, _cb_act, type, _view):
            ...
            content = content.format(
                ...
                type=type,
                runtime='&nbsp;' *
                (2 - len(str(self.runtime))) + str(self.runtime)
            )
            content = '<style>' + styles + '</style>' + content
    ...
    class Tester(object):
        ...
        def __init__(self, process_manager,
            ...
            if type(self.process_manager) != ProcessManager:
                self.process_manager.set_calls(
                    self.__on_out, self.__on_stop, on_status_change)
    ...
    def insert_text(self, edit, text=None):
        ...
        if len(v.sel()) > 1:
            return
        if v.sel()[0].a != expected or v.sel()[0].b != expected:
            return
        if text is None:
    ...
    def get_tie_pos(self, i):
        ...
        for j in range(i):
            ...
            if running:
                pt += len(tester.tests[j].test_string) + \
                    len(tester.prog_out[j]) + 1
    ...
    def on_test_action(self, i, event):
        ...
        if tester.proc_run and event in {'test-click', 'test-edit', 'test-run'}:
            sublime.status_message(
                'can not {action} while process running'.format(action=event))
            return
        if event == 'test-click':
            self.toggle_fold(i)
    ...
    def get_next_title(self):
        ...
        phantom = Phantom(Region(self.view.size() - 1),
                          content, sublime.LAYOUT_BLOCK, onclick)
        return phantom
    ...
    def update_configs(self, update_last=None):
        ...
        for i in range(k):
            ...
            if running:
                pt += len(tester.tests[i].test_string) + \
                    len(tester.prog_out[i]) + 2
            elif not tester.tests[i].fold:
                pt += len(tester.tests[i].test_string) + \
                    len(tester.prog_out[i]) + 1
        ...
        while len(self.test_phantoms) < len(configs):
            self.test_phantoms.append(PhantomSet(
                v, 'test-phantom-' + str(len(self.test_phantoms))))

        hide_phantoms = v.settings().get('hide_phantoms')
        if update_last:
            self.test_phantoms[_last_test_entry].update(
                [configs[_last_test_entry]] if not hide_phantoms else [])
        else:
            for i in range(len(configs)):
                self.test_phantoms[i].update(
                    [configs[i]] if not hide_phantoms else [])
    ...
    def new_test(self, edit):
        ...
        self.tester.next_test(
            v.size() - 1, lambda: self.update_configs(update_last=True))
    ...
    def memorize_tests(self):
        with open(get_tests_file_path(self.dbg_file), 'w') as f:
            f.write(sublime.encode_value([x.memorize()
                    for x in (self.tester.get_tests())], True))

    def on_insert(self, s):
        self.view.run_command(
            'test_manager', {'action': 'insert_opd_input', 'text': s})

    def on_out(self, s):
        v = self.view

        self.view.run_command(
            'test_manager', {'action': 'insert_opd_out', 'text': s})
    ...
    def add_region(self, line, region_prop):
        ...
        v.add_regions(str(randint(0, 1e9)), [
                      Region(pos.a, pos.a + 1)], *region_prop)
    ...
    def on_stop(self, rtcode, runtime, crash_line=None):
        ...
        v.add_regions('test_end_%d' % test_id,
                      [Region(self.input_start + len(_inp) + 1,
                              self.input_start + len(_inp) + 1)],
                      *self.REGION_END_PROP)
        ...
        if str(rtcode) == '0':
            if tester.running_new and tester.have_pretests():
                ...
                sublime.set_timeout(lambda: v.run_command(
                    'test_manager', {'action': 'new_test'}), 10)
            else:
            ...
            for x in v.window().views():
                if x.id() == self.code_view_id:
                    x.run_command('view_tester', {
                                  'action': 'show_crash_line', 'crash_line': crash_line})
    ...
    def fold_accept_tests(self):
        ...
        for i in range(cur_test):
            if self.tester.check_test(i):
                ...
                end = v.line(v.get_regions(self.REGION_END_KEY %
                             i)[0].begin()).end()
    ...
    def set_compile_bar(self, cmd, type=''):
        ...
        styles = get_test_styles(view)
        content = open(
            root_dir + '/Highlight/compile.html').read().format(cmd=cmd, type=type)
        content = '<style>' + styles + '</style>' + content
    ...
    def make_opd(self, edit, run_file=None, build_sys=None, clr_tests=False,
        ...
        v.set_scratch(True)
        v.run_command('set_setting', {
                      'setting': 'fold_buttons', 'value': False})
        v.run_command('set_setting', {
                      'setting': 'line_numbers', 'value': False})
        v.set_status('opd_info', 'opdebugger-file')
        self.clear_all()
        if load_session:
            if self.session is None:
                v.run_command('test_manager', {
                              'action': 'insert_opd_out', 'text': 'Can\'t restore session'})
            else:
        ...
        if not clr_tests:
            try:
                with open(get_tests_file_path(run_file)) as f:
                    tests = [self.Test(x) for x in sublime.decode_value(
                        f.read()) if x['test'].strip()]
            except:
                tests = []
        else:
    ...
    def delete_tests(self, edit):
        ...
        sublime.status_message('deleted tests: ' +
                               (', '.join(map(lambda x: str(x + 1), to_del))))
    ...
    def enable_edit_mode(self):
        ...
        if self.tester.proc_run:
            ...
            sublime.set_timeout_async(lambda: v.run_command(
                'test_manager', {'action': 'enable_edit_mode'}), 500)
            return
        ...
        for i in range(tests):
            ...
            if i == tests - 1:
                ...
                out_end = v.get_regions(
                    self.REGION_BEGIN_KEY % (i + 1))[0].begin()
            v.erase_regions(self.REGION_END_KEY % i)
            v.erase_regions('line_%d' % i)
            v.erase_regions('test_error_%d' % i)
            v.run_command('test_manager', {
    ...
    def swap_tests(self, edit, dir=-1):
        ...
        for i in range(len(tester.tests)):
            ...
            for reg in view.sel():
                if reg.intersects(Region(begin, end)):
                    ...
                    tester.tests[i].__sel.append(
                        Region(inter.a - begin, inter.b - begin))
                    break
        ...
        for sel in selected:
            if 0 <= sel + dir < len(tester.tests):
                tester.tests[sel], tester.tests[sel +
                                                dir] = tester.tests[sel + dir], tester.tests[sel]
                tester.prog_out[sel], tester.prog_out[sel +
                                                      dir] = tester.prog_out[sel + dir], tester.prog_out[sel]
...
class ModifiedListener(sublime_plugin.EventListener):
    ...
    def on_hover(self, view, point, hover_zone):
        if hover_zone == sublime.HOVER_TEXT:
            view.run_command(
                'view_tester', {'action': 'get_var_value', 'pos': point})
...
class ViewTesterCommand(sublime_plugin.TextCommand):
    ...
    def create_opd(self, clr_tests=False, sync_out=True, use_debugger=False):
            ...
            if get_settings().get('close_sidebar'):
                try:
                    sublime.set_timeout_async(
                        lambda window=window: window.set_sidebar_visible(False), 50)
                except:
        ...
        dbg_view.set_syntax_file(
            'Packages/%s/TestSyntax.tmLanguage' % base_name)
    ...
    def show_frames(self, frames=None):
        ...
        def on_highlight(id, frames=frames):
            pt = v.text_point(int(frames[id]['line']) - 1, 0)
            v.show_at_center(pt)
            v.add_regions('highlight', [v.line(pt)],
                          'variable.c++', 'dot', sublime.HIDDEN)

        v.window().show_quick_panel(items, on_select,
                                    sublime.MONOSPACE_FONT, 0, on_highlight)

    def show_var_value(self, value, pos=None):
        def nop(): pass
        self.view.show_popup(
            highlight(value), sublime.HIDE_ON_MOUSE_MOVE_AWAY, pos)
        ...
        if action == 'insert':
            ...
            if v.settings().get('syntax') == 'Packages/FastOlympicCoding/OPDebugger.tmLanguage':
                ...
                self.create_opd(clr_tests=clr_tests,
                                sync_out=sync_out, use_debugger=use_debugger)
    ...
    def move_syncer(self, view):
        try:
            ...
            if view.name()[-4:] == '-run':
                ...
                active_view_index = w.get_view_index(
                    w.active_view_in_group(0))[1]
